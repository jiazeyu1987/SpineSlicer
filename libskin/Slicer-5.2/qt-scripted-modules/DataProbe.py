...
class DataProbeInfoWidget:
    ...
    def processEvent(self, observee, event):
        ...
        for layer, logicCall in layerLogicCalls:
            if layer == 'B':
                ...
                v1 = '影像名称：'+self.generateLayerName(layerLogic)
                v2 = "坐标："+self.generateIJKPixelDescription(ijk, layerLogic)
                v3 = "像素值:"+self.generateIJKPixelValueDescription(ijk, layerLogic)
                self.layerNames[layer].setText(v1)
                self.layerIJKs[layer].setText(v2)
                self.layerValues[layer].setText(v3)
        ...
        if aggregatedDisplayableManagerInfo != '':
            aggregatedDisplayableManagerInfo = self.replace_segment(aggregatedDisplayableManagerInfo)
    ...
    def replace_segment(self, text):
        # 匹配 <font> 标签后面到 <br> 之前的内容
        pattern = r'(<font color="#[0-9a-fA-F]{6}">&#x25cf;</font>\s*)([a-zA-Z0-9\-]+)'

        # 替换函数
        def replace_match(match):
            original_segment = match.group(2)  # 获取匹配的内容
            result = util.tr(original_segment)  # 替换内容
            return f"{match.group(1)}{result}"  # 返回替换后的结果
    ...
    def generateViewDescription(self, xyz, ras, sliceNode, sliceLogic):
        ...
        return \
            "  {layoutName: <6s}({rLabel} {ras_x:3.1f}, {aLabel} {ras_y:3.1f}, {sLabel} {ras_z:3.1f})  层间隔: {spacing:s} 毫米" \
            .format(layoutName=self.get_chinese_layout_name(sliceNode.GetLayoutName()),
    ...
    def get_chinese_layout_name(self,val):
        if val == "Red":
            return "轴状面"
        if val == "Green":
            return "冠状面"
    ...
    def _createSmall(self):
        ...
        self.showImageBox = qt.QCheckBox('显示放大影像', self.showImageFrame)
        ...
        for (row, layer) in enumerate(layers):
            col = 0
            #layout.addWidget(qt.QLabel(layer), col, row)
            col += 1
            self.layerNames[layer] = qt.QLabel()
            layout.addWidget(self.layerNames[layer], col, row)
            col += 1
            self.layerIJKs[layer] = qt.QLabel()
            layout.addWidget(self.layerIJKs[layer], col, row)
            col += 1
            self.layerValues[layer] = qt.QLabel()
            layout.addWidget(self.layerValues[layer], col, row)
