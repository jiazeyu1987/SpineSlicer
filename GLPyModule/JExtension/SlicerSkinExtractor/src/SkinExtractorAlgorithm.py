...
def morphological_chan_vese_fillhole_2d_new(image: sitk.Image, orientation: str):
    ...
    util.send_event_str(util.ProgressStart,"分割第一阶段")
    ...
    for it in range(nb_iterations):
        ...
        util.send_event_str(util.ProgressValue,it)
        elapsed_time =  time.time() - start_time
        time_left = nb_iterations * elapsed_time / (it+1) - elapsed_time
        #progressDialog.setLabelText(f'时间剩余: {np.round(time_left,0)} 秒')
    ...
    #iterations = [25, 10, 5]
    iterations = [3, 1]
    util.send_event_str(util.ProgressStart,"分割第二阶段")
    util.send_event_str(util.ProgressValue,0)
    ...
    for it in range(len(iterations)):
        ...
        for _ in range(iterations[it]):
            ...
            util.send_event_str(util.ProgressValue,progress_bar_i*(100/maximum1))
            slicer.app.processEvents()
            elapsed_time =  time.time() - start_time
            time_left = sum(iterations) * elapsed_time / (progress_bar_i+1) - elapsed_time
            #progressDialog2.setLabelText(f'时间剩余: {np.round(time_left,0)} 秒。。')
            progress_bar_i+=1
    ...
    util.send_event_str(util.ProgressStart,"分割第三阶段")
    util.send_event_str(util.ProgressValue,0)
    ...
    #progressDialog3.setLabelText("正在准备数据")
    #progressDialog3.setValue(0)
    import slicer.util as util
    util.processEvents()
    progress_bar_i = 0
    maximum1=sum(iterations)
    ...
    for it in range(len(iterations)):
        ...
        for _ in range(iterations[it]):
            ...
            util.send_event_str(util.ProgressValue,progress_bar_i*int(100/maximum1))
            slicer.app.processEvents()
            elapsed_time =  time.time() - start_time
            time_left = sum(iterations) * elapsed_time / (progress_bar_i+1) - elapsed_time
           # progressDialog3.setLabelText(f'时间剩余: {np.round(time_left,0)} 秒。')
            progress_bar_i+=1
