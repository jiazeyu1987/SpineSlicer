...
class MyDataProbe(qt.QDialog):
  main_ui = None
  infoWidget = None
  def __init__(self, main, parent=None):
    super().__init__(parent)
    self.setWindowFlags(qt.Qt.Tool | qt.Qt.FramelessWindowHint)
    self.setAttribute(qt.Qt.WA_TranslucentBackground)  # 背景透明
    self.setAttribute(qt.Qt.WA_DeleteOnClose)  # 关闭时销毁

    self.module_path = os.path.dirname(slicer.util.modulePath("JMeasure"))
    print(self.module_path)
    uiWidget = slicer.util.loadUI(self.module_path + '/Resources/UI/MyDataProbe.ui')
    slicer.util.addWidget2(self, uiWidget)
    self.ui = slicer.util.childWidgetVariables(uiWidget)
    self.infoWidget = DataProbeInfoWidget(uiWidget)
    self.infoWidget.showImageBox.setChecked(True)
    util.addWidget2(self.ui.container, self.infoWidget.frame)
    self.start_pos = None  # 记录鼠标初始位置
    width = 420
    height = 400
    self.ui.MyDataProbe2.setMaximumHeight(height)
    self.ui.MyDataProbe2.setMinimumHeight(height)
    self.ui.MyDataProbe2.setMaximumWidth(width)
    self.ui.MyDataProbe2.setMinimumWidth(width)
    self.main_ui = main
    
    self.ui.btnClose.connect("clicked()",self.on_close)

  def on_close(self):
    self.main_ui.is_open_data_probe = False
    if self.infoWidget:
...
class JMeasureWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
  ...
  tool_bv_download = "tool_bv_download"
  use_tool_bar = 0
  layout_init = False
  button_width = 60
  button_height = 80
  ...
  def init_setting_panel_ui(self,parent):
    ...
    parent.registerProperty("General/bg_3d_color1", panelui.lineEdit1, "text", str(qt.SIGNAL("textChanged(QString)")))
    parent.registerProperty("General/bg_3d_color2", panelui.lineEdit2, "text", str(qt.SIGNAL("textChanged(QString)")))
  ...
  def get_control_panel(self):
    return self.ui.parent_widget1321
  ...
  def init_layout(self):
    ...
    if self.ui.parent_widget1321.layout() is None:
      if self.orientation == self.vertical:
        ...
        layout.addLayout(sub_layout2)
        sub_layout2.setSpacing(24)
        sub_layout2.setContentsMargins(0, -50, 50, 50)
    ...
    filepath = os.path.join(util.mainWindow().GetProjectBasePath(),"ProjectCache",project_name,"banner.png").replace('\\','/')
  ...
  def add_tool_by_name(self,tool_name,parent_widget1321):
    ...
    if tool_name == self.tool_slice:
      return self.add_tool_slice(parent_widget1321)
    if tool_name == self.tool_reset:
      return self.add_reset_tool(parent_widget1321)
    if tool_name == self.tool_probe:
      return self.add_probe_tool(parent_widget1321)
    ...
    if tool_name == self.tool_3Point1Plane:
      return self.add_tool_3Point1Plane(parent_widget1321)
    ...
    if tool_name == self.tool_bv_upload:
      return self.add_tool_bv_upload(parent_widget1321)
    if tool_name == self.tool_bv_download:
      return self.add_tool_bv_download(parent_widget1321)
    ...
    if tool_name == self.tool_system_info:
      return self.add_tool_system_info(parent_widget1321)
    ...
    if tool_name == self.tool_stand_view:
      return self.add_tool_stand_view(parent_widget1321)
  ...
  def add_multivolume_point_tool(self,parent):
    module_widget = util.getModuleWidget("JMultiVolumeTool")
    layout,btn = self.create_labeled_checkable_button(parent,"tool_dce_point.png","tool_dce_point.png","在DCE图像上取点","取点")
  ...
  def add_tool_test(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_clicked_zoomin.png","tool_clicked_zoomin.png","测试","测试")
  ...
  def add_tool_crosshairbox(self,parent):
    ...
    btn.setMenu(menu)
    self.add_icon_action(menu,"tool_cross.png","十字线",lambda:self.update_crosshairbox("tool_cross.png","十字线"))
    self.add_icon_action(menu,"tool_whirling.png","MPR",lambda:self.update_crosshairbox("tool_whirling.png","MPR"))
  ...
  def on_crosshairbox_mpr(self,is_show):
    ...
    slicer.app.applicationLogic().SetIntersectingSlicesEnabled(slicer.vtkMRMLApplicationLogic.IntersectingSlicesVisibility, is_show)
    slicer.app.applicationLogic().SetIntersectingSlicesEnabled(slicer.vtkMRMLApplicationLogic.IntersectingSlicesInteractive,is_show)
  ...
  def add_tool_measurebox(self,parent):
    ...
    menu = qt.QMenu(parent)
    btn.setMenu(menu)
    slicer.app.applicationLogic().GetInteractionNode().AddObserver(slicer.vtkMRMLInteractionNode.InteractionModeChangedEvent, self.OnInteractionModeChangedEvent)
    self.add_icon_action(menu,"tool_line.png","长度",lambda:self.update_measure_box(btn,"tool_line.png","长度"))
    self.add_icon_action(menu,"tool_angle.png","角度",lambda:self.update_measure_box(btn,"tool_angle.png","角度"))
    self.add_icon_action(menu,"tool_area.png","面积",lambda:self.update_measure_box(btn,"tool_area.png","面积"))
    self.add_icon_action(menu,"tool_curve.png","曲线",lambda:self.update_measure_box(btn,"tool_curve.png","曲线"))
    self.add_icon_action(menu,"tool_rectangle.png","矩形",lambda:self.update_measure_box(btn,"tool_rectangle.png","矩形"))
  ...
  def on_delete_all_measure(self):
    res = util.messageBox("确定删除所有测量项吗",windowTitle=util.tr("提示"))
    if res == 0:
      return
    nodes = util.getNodesByClass(util.vtkMRMLMarkupsNode)
    for node in nodes:
  ...
  def add_tool_measurebox_outside(self,parent,btn):
    ...
    btn.setMenu(menu)
    slicer.app.applicationLogic().GetInteractionNode().AddObserver(slicer.vtkMRMLInteractionNode.InteractionModeChangedEvent, self.OnInteractionModeChangedEvent)
    self.add_icon_action(menu,"tool_line.png","长度",lambda:self.update_measure_box_outside(btn,"tool_line.png","长度"))
    self.add_icon_action(menu,"tool_angle.png","角度",lambda:self.update_measure_box_outside(btn,"tool_angle.png","角度"))
    self.add_icon_action(menu,"tool_area.png","面积",lambda:self.update_measure_box_outside(btn,"tool_area.png","面积"))
    self.add_icon_action(menu,"tool_curve.png","曲线",lambda:self.update_measure_box_outside(btn,"tool_curve.png","曲线"))
    self.add_icon_action(menu,"tool_rectangle.png","矩形",lambda:self.update_measure_box_outside(btn,"tool_rectangle.png","矩形"))
    self.add_icon_action(menu,"tool_roi.png","ROI",lambda:self.update_measure_box_outside(btn,"tool_roi.png","ROI"))
  ...
  def get_extension(self):
    ...
    with open(util.mainWindow().GetProjectIniPath().replace("\\","/"), encoding='utf-8') as f:
        config.read_file(f)
        f.close()
    scene_extension = util.getjson("scene_extension","mrb")
  ...
  def add_tool_developer(self,parent):
    ...
    menu.addAction("保存",lambda:util.saveScene(slicer.app.applicationDirPath()+"/bak/save.%s")%(self.get_extension()))
  ...
  def change_to_test_style(self,k,style):
    ...
    if self.orientation == "horizontal":
      ...
      util.show_right_style(right_module=style,middle_right_module="JMeasure",right_width=500,middle_right_width=58,full_screen=True)
  ...
  def add_tool_exit(self,parent):
    ...
    return tool
  def on_exit(self):
    res = util.messageBox("退出将损失当前的所有数据,确认要退出当前操作吗",windowTitle=util.tr("提示"))
    if res == 0:
        return
    util.quit()   
  ...
  def add_tool_close_scene(self,parent):
    layout,btn = self.create_labeled_clicked_button(parent,"tool_close_scene.png","删除当前的所有数据,将软件重置为初始化状态","清除")
  ...
  def on_save(self):
    ...
    if util.isPackage():
      ...
      filepath = os.path.join(util.mainWindow().GetProjectBasePath(),"ProjectCache",project_name,"saves",util.username,timestamp_str+"."+self.get_extension()).replace('\\','/')
    ...
    if util.getjson2("Markups","use_scene_view") == "2":
      scene_node_name = "main_scene_view_save_node"
      scene_node = util.getFirstNodeByName(scene_node_name)
      if scene_node:
        util.RemoveNode(scene_node)
      #screenfilepath = os.path.join(util.mainWindow().GetProjectBasePath(),"ProjectCache",project_name,"saves",util.username,timestamp_str+".png").replace('\\','/')
      
      util.getModuleLogic("SceneViews").CreateSceneView(scene_node_name,"abc",1,vtk.vtkImageData()) 
      # scene_node = util.getFirstNodeByName(scene_node_name)
      # scene_node.AddDefaultStorageNode()
      # volumeStorageNode = scene_node.GetStorageNode()
      # volumeStorageNode.SetFileName("D:/11123.png")
      # scene_node.StorableModified()
    
    database_path = os.path.join(util.mainWindow().GetProjectBasePath(),"Resources","Database",util.username,"ctkDICOM.sql")
    # 连接到数据库
    if os.path.exists(database_path):
      ...
      cursor = conn.cursor()
      cursor.execute('INSERT INTO Analysis (patient_name, solution_name,pathlist,recordtime,modifytime) VALUES (?, ?,?,?,?)', (util.patient_name,util.solution_name,filepath,datetime.now(),datetime.now()))
      conn.commit()
      # 关闭连接
      conn.close()
    print("on save scene to path:",filepath)
  ...
  def add_tool_page_volumerendering(self,parent):
    layout,btn = self.create_labeled_clicked_button(parent,"tool_page_volumerendering.png","进入三维重建页面","渲染")
  ...
  def on_load(self):
    ...
    if util.JsonData is None:
      filepath = (os.path.join(util.mainWindow().GetProjectBasePath(),"Resources","save.%s")%(self.get_extension())).replace('\\','/')
    else:
      project_name = util.getjson("project_name")
      filepath = (os.path.join(util.mainWindow().GetProjectBasePath(),"ProjectCache",project_name,"save.%s")%(self.get_extension())).replace('\\','/')
    ...
    if util.getjson2("Markups","use_scene_view") == "2":
      ...
      if scene_view_node:
        try:
          ...
          print("TraceBack:scene view bug here,it's not important")
    tool_usb = util.GetGlobalSaveValue("tool_usb")
    tool_ime = util.GetGlobalSaveValue("tool_ime")
    tool_robot = util.GetGlobalSaveValue("tool_robot")
    tool_whirling = util.GetGlobalSaveValue("tool_whirling")
    if tool_usb == "True":
      self.get_tool_by_name("usb").btn.setChecked(True)
    if tool_ime == "True":
      self.get_tool_by_name("tool_ime").btn.setChecked(True)
    if tool_robot == "True":
      self.get_tool_by_name("tool_elibot").btn.setChecked(True)
    if tool_whirling == "True":
      self.get_tool_by_name("whirling").btn.setChecked(True)

  def add_tool_layout(self,parent):
    layout,btn = self.create_toolbutton(parent,"tool_layout.png","更改布局",util.tr("布局"))
    menu = qt.QMenu(parent)
    btn.setMenu(menu)
    first_id = 2
    menu.addAction(util.tr("上一下3"),lambda a: self.on_choose_layout(a,2))
    menu.addAction(util.tr("左一右3"),lambda a: self.on_choose_layout(a,16))
    menu.addAction(util.tr("左1右2"),lambda a: self.on_choose_layout(a,27))
    menu.addAction(util.tr("四视图"),lambda a: self.on_choose_layout(a,3))
  ...
  def on_choose_layout(self,a,page):
    ...
    if util.get_from_PAAA("current_project_selector_project_name") == "PUltraSoundPlan":
  ...
  def add_whirling_tool(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_whirling.png","tool_whirling.png","拖动十字线进行旋转","MPR")
  ...
  def on_whirling(self,is_show):
      util.SetGlobalSaveValue("tool_whirling",is_show)
  ...
  def add_tool_slice(self,parent):
    layout,btn = self.create_toolbutton(parent,"btn_all_tube_slice.png",util.tr("截面"),util.tr("截面"))
    btn.setCheckable(True)
    btn.connect('toggled(bool)',self.on_tool_slice)
    menu = qt.QMenu(parent)
    btn.setMenu(menu)
    self.add_icon_action(menu,"tool_layout1.png",util.tr("矢状面"),lambda:self.on_sub_tool_slice("矢状面",not btn.isChecked()))
    self.add_icon_action(menu,"tool_layout2.png",util.tr("冠状面"),lambda:self.on_sub_tool_slice("冠状面",not btn.isChecked()))
    self.add_icon_action(menu,"tool_layout3.png",util.tr("水平面"),lambda:self.on_sub_tool_slice("水平面",not btn.isChecked()))
    tool = JMeasureTool(layout,btn,"tool_slice")
    self.tool_list.append(tool)
    return tool
  def on_tool_slice(self,is_show):
      print("on_tool_slice:",is_show)
      layoutManager = slicer.app.layoutManager()
      names = ["Red","Yellow","Green"]
      for sliceViewName in names:
        view = layoutManager.sliceWidget(sliceViewName).sliceView()
        sliceNode = view.mrmlSliceNode()
        sliceNode.SetSliceVisible(is_show)
  
  def on_normal(self):
        util.trigger_view_tool("")


  def on_sub_tool_slice(self,name,is_show):
    layoutManager = slicer.app.layoutManager()
    names = ["Red","Yellow","Green"]
    for sliceViewName in names:
    ...
    if name == "冠状面":
      view = layoutManager.sliceWidget("Green").sliceView()
    if name == "矢状面":
      view = layoutManager.sliceWidget("Yellow").sliceView()
    if name == "水平面":
  ...
  def add_WindowLevel_tool(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_grayscale.png","tool_grayscale.png","框选二维视图中感兴趣的区域来更改窗宽窗位","灰度")
  ...
  def add_reset_tool(self,parent):
    ...
    if util.curr_language == 1:
      btn_width = 30
    layout,btn = self.create_labeled_clicked_button(parent,"tool_reset.png","将所有窗口的显示重置为初始状态",util.tr("重置"), btn_width)
    tool = JMeasureTool(layout,btn,"reset")
    self.tool_list.append(tool)
    btn.connect('clicked()', self.on_reset)
    return tool
  def on_reset(self):
    util.reinitForce(ori=True)
    util.trigger_view_tool("")
    util.forceRenderAllViews()
    
  def add_probe_tool(self,parent):
    btn_width = 0
    if util.curr_language == 1:
      btn_width = 30
    layout,btn = self.create_labeled_clicked_button(parent,"tool_probe.png","",util.tr("探针"), btn_width)
    tool = JMeasureTool(layout,btn,"probe")
    self.tool_list.append(tool)
    btn.connect('clicked()', self.on_probe)
    return tool
  def on_probe(self):
    if self.is_open_data_probe:
      return
    self.is_open_data_probe = True
    dialog = MyDataProbe(self, slicer.util.mainWindow())
    dialog.show()
  '''
    显示窗宽窗位的头骨部分
  '''
  ...
  def add_WindowLevel_tissue(self,parent):
    layout,btn = self.create_labeled_clicked_button(parent,"tool_windowlevel_tissue.png","显示软组织的窗宽窗位","软组织窗")
  ...
  def add_line_tool(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_line.png","tool_line.png","测量长度的工具",util.tr("线段"))
  ...
  def OnMarkupsComplete(self,markups_node):
    ...
    if markups_node.GetAttribute("scene_view_node_id") is None:
      if util.getjson2("Markups","use_scene_view") == "2":
        ...
        util.getModuleLogic("SceneViews").CreateSceneView(scene_view_node_name,"measure scene view",4,vtk.vtkImageData()) 
      ...
      slicer.app.applicationLogic().GetInteractionNode().RemoveObserver(slicer.vtkMRMLInteractionNode.InteractionModeChangedEvent)
      slicer.app.applicationLogic().GetInteractionNode().AddObserver(slicer.vtkMRMLInteractionNode.InteractionModeChangedEvent, self.OnInteractionModeChangedEvent)
    util.trigger_view_tool("")
  ...
  def add_angle_tool(self,parent):
    ...
    if util.curr_language == 1:
      btn_width = 30
    layout,btn = self.create_labeled_checkable_button(parent,"tool_angle.png","tool_angle.png","测量角度的工具",util.tr("角度"), btn_width)
  ...
  def add_area_tool(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_area.png","tool_area.png","测量面积的工具",util.tr("面积"))
  ...
  def add_area_open_tool(self,parent):
    ...
    if util.curr_language == 1:
      btn_width = 30
    layout,btn = self.create_labeled_checkable_button(parent,"tool_curve.png","tool_curve.png","测量长度的工具,左键选择曲线,右键取消",util.tr("曲线"), btn_width)
  ...
  def add_tool_3Point1Plane(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"contact_wechat.png","contact_wechat.png","通过三点创建一个平面","3P1P")
    tool = JMeasureTool(layout,btn,"3P1P")
    self.tool_list.append(tool)
    btn.connect('toggled(bool)', self.on_tool_3Point1Plane)
    return tool
  def on_tool_3Point1Plane(self,is_show):
    widget = util.getModuleWidget("ThreePointOnePlane").ui.CreatePlane
    if is_show:
      qwidget = qt.QWidget()
      qwidget.setParent(slicer.app.layoutManager().sliceWidget("Red"))
      util.addWidget2(qwidget,widget)
      qwidget.geometry = qt.QRect(16,6,300,170)
      qwidget.show()
    else:
  ...
  def add_tool_screenshot(self,parent):
    ...
    if util.curr_language == 1:
      btn_width = 65
    layout,btn = self.create_labeled_clicked_button(parent,"tool_screen_shot.png",util.tr("截图"),util.tr("截图"), btn_width)
  ...
  def add_tool_controller(self,parent):
    layout,btn = self.create_labeled_clicked_button(parent,"tool_show_controller.png","是否显示四视图上的控制栏","控制栏")
  ...
  def add_tool_tick(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_show_controller.png","tool_show_controller.png","Tick","Tick")
  ...
  def on_tool_tick(self,is_show):
    ...
    if is_show:
      ...
      util.getModuleWidget("RequestStatus").send_cmd(f"Elibot, SetGapMax, 1")
      util.getModuleWidget("RequestStatus").send_cmd(f"IME, SetGapMax, 1")
      util.getModuleWidget("RequestStatus").send_cmd(f"Register, SetGapMax, 10000000000")
      util.getModule("Tick").start_thread(0.1)
    else:
      util.getModule("Tick").pause_thread()

  def add_tool_system_info(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_light_on.png","tool_light_off.png","系统","系统")
    tool = JMeasureTool(layout,btn,"tool_system_info")
    self.tool_list.append(tool)
    btn.setCheckable(True)
    #util.singleShot(2000,lambda:btn.setChecked(True))
    btn.connect('toggled(bool)', self.on_tool_system_info)
    return tool
  def on_tool_system_info(self,is_show):
    #util.showWarningText("正在进行手眼标定")
    widget = util.getModuleWidget("JSystemInfo").ui.widget
    if is_show:
      qwidget = qt.QWidget()
      qwidget.setParent(slicer.app.layoutManager().threeDWidget(0).threeDView())
      util.addWidget2(qwidget,widget)
      qwidget.geometry = util.getModuleWidget("RDNTreat").treat_layout.get_system_info_layout()
      qwidget.show()
    else:
      widget.setParent(None)

  def get_ui_multiwidget(self):
    id = 3
    if id == 1:
      return slicer.app.layoutManager().sliceWidget("Red+")
    elif id == 2:
      layoutManager=slicer.app.layoutManager()
      for threeDViewIndex in range(layoutManager.threeDViewCount) :
        widget = slicer.app.layoutManager().threeDWidget(threeDViewIndex)
        if widget.width>1500:
          return widget
    elif id==3:
      return slicer.app.layoutManager().sliceWidget("Green+")
  
  def add_tool_ime(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_light_on.png","tool_light_off.png","NDI显示","NDI")
  ...
  def on_tool_ime(self,is_show):
    #util.showWarningText("正在进行手眼标定")
    util.SetGlobalSaveValue("tool_ime",is_show)
    from slicer import app
    widget = util.get_double_camera_widget()
    if is_show:
      qwidget = qt.QWidget()
      qwidget.setParent(self.get_ui_multiwidget())
      util.addWidget2(qwidget,widget)
  ...
  def add_tool_usb(self,parent):
    ...
    btn.setCheckable(True)
    btn.connect('toggled(bool)',self.on_tool_UltrasoundGenerator)
    menu = qt.QMenu(parent)
    btn.setMenu(menu)
    self.add_icon_action(menu,"tool_cross.png","功放",lambda:self.on_usb_tool("功放",not btn.isChecked()))
    self.add_icon_action(menu,"tool_whirling.png","电源",lambda:self.on_usb_tool("电源",not btn.isChecked()))
    self.add_icon_action(menu,"tool_whirling.png","水处理",lambda:self.on_usb_tool("水处理",not btn.isChecked()))
    tool = JMeasureTool(layout,btn,"usb")
    self.tool_list.append(tool)
    return tool
  def on_usb_tool(self,name,is_show):
    if name == "功放":
      layout = util.getModuleWidget("RDNTreat").treat_layout.get_usb_layout(0)
      widget = util.getModuleWidget("JUltrasoundGenerator").ui.widget
    if name == "电源":
      layout = util.getModuleWidget("RDNTreat").treat_layout.get_usb_layout(2)
      widget = util.getModuleWidget("JPowerControl").ui.widget
    if name == "水处理":
      layout = util.getModuleWidget("RDNTreat").treat_layout.get_usb_layout(1)
      widget = util.getModuleWidget("JWaterControl").ui.widget
    
    if is_show:
      qwidget = qt.QWidget()
      qwidget.setParent(self.get_ui_multiwidget())
      util.addWidget2(qwidget,widget)
      qwidget.geometry = layout
      qwidget.show()
    else:
      widget.setParent(None)
    
  def on_tool_UltrasoundGenerator(self,is_show):
    util.SetGlobalSaveValue("tool_usb",is_show)
    self.on_usb_tool("功放",is_show)
    self.on_usb_tool("电源",is_show)
    self.on_usb_tool("水处理",is_show)
  
  def add_tool_stand_view(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_stand_view.png","tool_stand_view.png","标准视图","STV")
    tool = JMeasureTool(layout,btn,"tool_stand_view")
    self.tool_list.append(tool)
    btn.setCheckable(True)
    #util.singleShot(2000,lambda:btn.setChecked(True))
    btn.connect('toggled(bool)', self.on_tool_stand_view)
    return tool
  
  def on_tool_stand_view(self,val):
    if val:
      util.getModuleWidget("StandView").show_dialog()
  
  def add_tool_elibot(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_scale_on.png","tool_scale_off.png","机械臂控制面板","机械臂")
  ...
  def on_tool_elibot(self,is_show):
    util.SetGlobalSaveValue("tool_robot",is_show)
    #util.showWarningText("正在进行手眼标定")
    widget = util.get_robot_arm_widget()
    if is_show:
      qwidget = qt.QWidget()
      qwidget.setParent(self.get_ui_multiwidget())
      util.addWidget2(qwidget,widget)
  ...
  def add_tool_light(self,parent):
    ...
    if util.curr_language == 1:
      btn_width = 30
    layout,btn = self.create_labeled_checkable_button(parent,"tool_light_on.png","tool_light_off.png","添加渲染阴影",util.tr("光照"), btn_width)
    tool = JMeasureTool(layout,btn,"tool_light")
    self.tool_list.append(tool)
    btn.connect('toggled(bool)', self.on_tool_hide_light)
    self.on_tool_hide_light(False)
    return tool
  ...
  def add_tool_scale(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_scale_on.png","tool_scale_off.png","最大化视图区域","展示")
  ...
  def add_tool_bv_upload(self,parent):
    layout,btn = self.create_labeled_clicked_button(parent,"volume_register.png","上传数据","上传")
    tool = JMeasureTool(layout,btn,"bv_upload")
    self.tool_list.append(tool)
    btn.connect('clicked()', self.on_tool_bv_upload)
    return tool
  def on_tool_bv_upload(self):
    util.getModuleWidget("JUITool").on_popup_modulepanel_dialog("上传数据","BrainPlatformTaskUploader",width=600,height=600)

  
  def add_tool_bv_download(self,parent):
    layout,btn = self.create_labeled_clicked_button(parent,"tool_data_loader.png","下载数据","下载")
    tool = JMeasureTool(layout,btn,"bv_download")
    self.tool_list.append(tool)
    btn.connect('clicked()', self.on_tool_bv_download)
    return tool
  def on_tool_bv_download(self):
    util.getModuleWidget("JUITool").on_popup_modulepanel_dialog("下载数据","BrainPlatformTaskDownloader",width=600,height=600)
    
    
  def add_tool_save_volume_paras(self,parent):
    layout,btn = self.create_labeled_clicked_button(parent,"tool_volumerendering_recorder.png","记录体绘制的参数","体参")
  ...
  def add_hide_measure_tool(self,parent):
    layout,btn = self.create_labeled_checkable_button(parent,"tool_visible_on.png","tool_visible_on.png","隐藏所有的测量项","隐藏")
  ...
  def on_tool_screenshot(self):
    fileName = qt.QFileDialog.getSaveFileName(None, ("保存截图"),
                            ...
                            ("截图 (*.png)"))
    if fileName == "":
      util.showWarningText("请选择一个文件地址")
      return
  ...
  def get_big_icon_panel(self,style=0):
    ...
    if style==1:
      ...
      for data in data["paras"]:
        sub_json_data = data
        menu.addAction(f"模式{index}",lambda a,v1=index,v2=sub_json_data: jm.on_choose_volume_paras(a,v1,v2))
        index+=1
  ...
  def create_labeled_clicked_button(self,parent,picture_name,tooltip,label,btn_width=0,icon_width=20,rlist=None):
    import qt
    if rlist:
    ...
    labelText.setObjectName("labelText")
    labelText.setStyleSheet("font: 12px 'Source Han Sans CN-Regular, Source Han Sans CN';background-color: transparent;")
    if btn_width == 0:
      btn_width = self.button_width
    if self.orientation == self.horizontal:
      layout = qt.QVBoxLayout()
      btn.setFixedSize(btn_width,self.button_height)
    else:
      layout = qt.QVBoxLayout()
    ...
    btn_stylesheet = btn_stylesheet + "QPushButton:hover{background-color: #243B5A;}"
    btn_stylesheet = btn_stylesheet + "QPushButton:pressed{background-color: #2B649A;}"
  ...
  def create_labeled_checkable_button(self,parent,checked_false_name,checked_true_name,tooltip,label,btn_width=0,icon_width=20):
    import qt
    self.resourcelist[checked_false_name] = tooltip
    btn = qt.QPushButton(parent)
    print("whm test", label, btn_width)
    if btn_width == 0:
      btn_width = self.button_width
    ...
    stylesheet = btn.styleSheet
    stylesheet = stylesheet + "QPushButton{border: 0px}"
    stylesheet = stylesheet + "QPushButton:checked{background-color: #2B649A;}"
    btn.setStyleSheet(stylesheet)
    return btn,btn


  def create_toolbutton(self,parent,picture_name,tooltip,label,icon_width=24):
    import qt
    self.resourcelist[picture_name] = tooltip
    if self.orientation == self.horizontal:
      ...
      labelText = qt.QLabel()
      labelText.setText(label)
      labelText.setAlignment(0x0004|0x0080)
      labelText.setObjectName("labelText")
      labelText.setStyleSheet("font: 12px 'Source Han Sans CN-Regular, Source Han Sans CN';background-color: transparent;")
      btn = qt.QToolButton()   
      btn.setLayoutDirection(qt.Qt.LeftToRight)
      ...
      layout_btn.setFixedSize(self.button_width+15,self.button_height)
      btn.setFixedSize(self.button_width+15,24)
      
      layout.setContentsMargins(0, 0, 0, 0)
      layout.setAlignment(0x0004|0x0080)
      layout.addWidget(btn)
      layout.addSpacing(self.layout_space-1)
      layout.addWidget(labelText)
      layout.addStretch()
      ...
      btn_stylesheet2 = btn_stylesheet2 + "QToolButton:hover{background-color: #243B5A}"
      ...
      btn_stylesheet2 = btn_stylesheet2 + "QToolButton:checked{background-color: #2B649A}"
      ...
      btn_stylesheet = btn.styleSheet
      btn_stylesheet = btn_stylesheet + "QToolButton:hover{background-color: #243B5A;}"
      btn_stylesheet = btn_stylesheet + "QToolButton:checked{background-color: #2B649A}"
      btn.setStyleSheet(btn_stylesheet)
      return btn,btn
