...
curr_language = 0
is_load_from_storge = False
#translate = qt.Qt.QCoreAppliction.translate
...
def get_icon_png():
  ...
  path = os.path.join(get_project_base_path(),"ProjectCache",project_name_info,"logo.png").replace("\\","/")
  return path
...
def login_required(func):
  def wrapper(*args, **kwargs):
      if username == "游客":
        showWarningText("请先登录您的账号")
        getModuleWidget("FrameworkTop").on_login_logout()
        return -1
      else:
        print("user name current is:",username)
      result = func(*args, **kwargs)
      return result
  return wrapper

def vip_jump(isGuest):
  if isGuest:
    dialog = getModuleWidget("JMessageBox").on_popup_ui_dialog("登录",getModuleWidget("FrameworkLogin").get_widget(), 700, 450)
    getModuleWidget("FrameworkLogin").set_dialog(dialog)
    dialog.exec_()
  else:
    show_vip()
...
def vip_required(func):
  def wrapper(*args, **kwargs):
      ...
      if viplevel != 1:
        showWarningText("当前功能需要VIP")
        is_guest = (username == "游客")
        vip_jump(is_guest)
        return
      result = func(*args, **kwargs)
      return result
...
DisplayModelAlias = 3000001
HideModelAlias = 3000002
ClearPatientInfo = 3000003
UpdateVIPTime = 3000004
UpdateTitle = 3000005

BackToSolution = 3000006
SaveSolution = 3000007
BeginSaveSolution = 3000008
...
def send_event_without_node(key,value=1):
  import slicer
  slicer.mrmlScene.InvokeEvent(key)


def get_segmentid_by_segment(segmentation_node,segment):
  if not segmentation_node:
    return None
  if not segment:
    return None
  return segmentation_node.GetSegmentation().GetSegmentIdBySegment(segment)

def hidden_by_name(nodename,val):
  node = getFirstNodeByName(nodename)
  if not node:
    return
  hidden(node,val)

def hidden(node,boolval):
  if node is None:
    return
  if boolval:
    node.SetHideFromEditors(1)
    node.SetAttribute("vtkMRML.Node.HideFromEditors", "1")
  else:
    node.SetHideFromEditors(0)
    node.SetAttribute("vtkMRML.Node.HideFromEditors", "0")
...
class ProgressTaskCliUnknown():
  ...
  def run(self,tag="请稍候"):
    ...
    send_event_str(ProgressStart,"请稍候")
  ...
  def onProgress(self):
    ...
    if self.target_finished_flag:
      ...
      if step > 1:
        step = 1
    self.inner_index += step
    util.send_event_str(ProgressValue,(int)(self.inner_index))
    if self.run_flag:
      qt.QTimer.singleShot(100, lambda: self.onProgress())
    else:
      util.send_event_str(ProgressValue,100)
      if self.target_finished_flag:
        self.success_func()
    slicer.app.processEvents()
...
def get_world_control_point_by_name(name):
    p1 = [0,0,0]
    pointnode = getFirstNodeByName(name)
    if not pointnode:
      return None
    pointnode.GetNthControlPointPositionWorld(0,p1)
    return p1

def create_plane_by_two_point(pointlist,plane_name):  
    import numpy as np
    if len(pointlist) < 2:
      showWarningText("至少需要3个点来创建平面")
      return
    origin = [0,0,0]
    for i in range(3):
      total = 0
    ...
    planeNode = getFirstNodeByName(plane_name)
    if planeNode:
      RemoveNode(planeNode)
    planeNode = AddNewNodeByNameByClass(plane_name,vtkMRMLMarkupsPlaneNode)
    GetDisplayNode(planeNode).SetNormalVisibility(False)
    GetDisplayNode(planeNode).SetPropertiesLabelVisibility(False)
    p1 = pointlist[0]
    p2 = pointlist[1]
    normal = np.array(p1)/2 -  np.array(p2)/2
    # 使用三个标记点设置平面的位置和方向
...
def rotate_to_plane(slice_name,plane_name):
    import slicer
    ...
    if not planenode:
      return
    normal = list(planenode.GetNormal())
    layoutManager = slicer.app.layoutManager()
    slice_node = layoutManager.sliceWidget(slice_name).mrmlSliceNode()
    matrix = slice_node.GetSliceToRAS()
    pos = [matrix.GetElement(0,3),matrix.GetElement(1,3),matrix.GetElement(2,3)]
    setSlicePoseFromSliceNormalAndPosition(slice_node,normal,pos)
    ...
    yellow_node = layoutManager.sliceWidget(name2list[0]).mrmlSliceNode()
    blue_node = layoutManager.sliceWidget(name2list[1]).mrmlSliceNode()
    setSlicePoseFromSliceNormalAndPosition(yellow_node,v1,pos)
    setSlicePoseFromSliceNormalAndPosition(blue_node,v2,pos)
    
    
def create_plane_by_three_point(pointlist,plane_name):
    import numpy as np
    if len(pointlist) < 3:
      showWarningText("至少需要3个点来创建平面")
      return
...
def show_left_right_style(left_width=395,right_width=395,docker_margin=10,left_module="Data",full_screen=True,right_module="TestScript"):
  import slicer
  ...
  layout_center_widget().setGeometry(0,0,rect.width()-left_width-right_width,rect.height()-taskbar_height-title_height)
...
def show_left_style(left_width=495,docker_margin=10,measure_height=68,top_docker_height=56,middle_right_width=210,middle_bottom_height=130,middle_top_module=None,middle_right_module=None,middle_bottom_module=None,top_module=None,left_module=None,full_screen=False,show_main=True):
  import slicer
  available_screenRect = slicer.app.desktop().availableGeometry()
  rect = mainWindow().GetScreenSize()
  title_height = slicer.app.style().pixelMetric(26)
  ...
  if middle_right_module:
    ...
    layout_panel("middle_right").setGeometry(0,0,middle_right_width,rect.height()-top_docker_height-docker_margin)
  ...
  if middle_top_module:
    ...
    layout_panel("middle_top").setGeometry(middle_right_width,0,rect.width()-left_width-docker_margin,measure_height)
  else:
    measure_height = 0

  if middle_bottom_module:
    layout_panel("middle_bottom").setModule(middle_bottom_module)
    layout_panel("middle_bottom").setGeometry(middle_right_width,rect.height()-top_docker_height-docker_margin-middle_bottom_height,rect.width()-left_width-docker_margin-middle_right_width,middle_bottom_height)
  ...
  if left_module:
    layout_panel("left").setModule(left_module)
    layout_dock("left").setFixedSize(left_width,rect.height()-taskbar_height-title_height-top_docker_height-docker_margin)
    layout_panel("left").show()
    layout_dock("left").show()
  else:
    left_width=0
    docker_margin=0
    layout_border_widget().setGeometry(0,0,rect.width()-left_width-docker_margin,rect.height()-taskbar_height-title_height-top_docker_height-docker_margin)
    layout_center_widget().setGeometry(0,measure_height,rect.width()-left_width-docker_margin,rect.height()-measure_height-taskbar_height-title_height-top_docker_height-docker_margin-middle_bottom_height)
...
def get_resource(resource_name,size=48,use_default_path=True):
  import os
  style = get_from_PAAA("slicer_style",default="Dark Slicer")
  if style == "Dark Slicer":
    keystyle = "Dark"
  else:
    keystyle = "Light"
  project_name_info = get_from_PAAA("current_project_selector_project_name")
  path = os.path.join(get_project_base_path(),"ProjectCache",project_name_info,"Icons",keystyle).replace("\\","/")
  if not os.path.exists(path):
    os.makedirs(path)
  fullname = path+"/"+resource_name
  if not os.path.exists(fullname):
    if use_default_path:
      fullname = os.path.join(get_project_base_path(),"ProjectCache",project_name_info,"Icons",keystyle,f"empty{size}.png").replace("\\","/")
...
def show_left_research_style(left_module=None,middle_top_module=None,middle_right_width=47,middle_right_module=None,show_main=True,left_width=400,show_toolbar=True):
  import qt,slicer
...
def show_research_style(left_or_right="right",in_module=None,middle_top_module=None,show_main=True,width=400,show_toolbar=True):
  import qt
  if show_toolbar:
    setToolbarsVisible(True)
    setMenuBarsVisible(True)
...
def show_3D_Orientation_Marker(marker_type):
  if marker_type > 3 or marker_type < 0:
    marker_type = 0
  import slicer
  threeDWidget = slicer.app.layoutManager().threeDWidget(0)
  # 隐藏掉方位标识OrientationMarker:0,不显示;1,Cube;2,Human;3,Axes
  threeDWidget.mrmlViewNode().SetOrientationMarkerType(marker_type)
  threeDWidget.mrmlViewNode().SetOrientationMarkerSize(0)
...
def set_controller_vertical_layout():
  ...
  #   qMRMLThreeDViewControllerWidget.alignment = 34
    threeDWidget = slicer.app.layoutManager().threeDWidget(0)
    # 隐藏掉方位标识OrientationMarker:0,不显示;1,Cube;2,Human;3,Axes
    #threeDWidget.mrmlViewNode().SetOrientationMarkerType(1)
    ...
    qMRMLThreeDViewControllerWidget.alignment = 34
    #show_3D_Orientation_Marker(1)

def show_right_style(right_width=495,right_only=False,docker_margin=10,measure_height=68,top_docker_height=55,middle_right_width=47,middle_bottom_height=130,middle_top_module=None,middle_right_module=None,middle_bottom_module=None,top_module=None,right_module=None,full_screen=False,show_main=True,fixed=True,right_module_title=None,right_module_title_height=40,show_toolbar=False):
  import slicer,qt
  if show_toolbar:
    setToolbarsVisible(True)
    setMenuBarsVisible(True)
...
def show_right_style2(right_width=495,right_only=False,docker_margin=10,measure_height=68,top_docker_height=55,middle_right_width=210,middle_bottom_height=130,middle_top_module=None,middle_right_module=None,middle_bottom_module=None,top_module=None,right_module=None,full_screen=False,show_main=True):
  import slicer
  ...
  if middle_right_module:
    layout_panel("middle_right").setModule(middle_right_module)
    layout_panel("middle_right").setGeometry(rect.width()-right_width-docker_margin-middle_right_width,0,middle_right_width,rect.height()-top_docker_height-docker_margin)
    layout_panel("middle_right").show()
  else:
    middle_right_width=0
  ...
  if middle_bottom_module:
    layout_panel("middle_bottom").setModule(middle_bottom_module)
    layout_panel("middle_bottom").setGeometry(0,rect.height()-top_docker_height-docker_margin-middle_bottom_height,rect.width()-right_width-docker_margin-middle_right_width,middle_bottom_height)
    layout_panel("middle_bottom").show()
  else:
    middle_bottom_height = 0
  ...
  if right_module:
    ...
    right_width=0
    docker_margin=0
    layout_border_widget().setGeometry(0,0,rect.width()-right_width-docker_margin,rect.height()-taskbar_height-title_height-top_docker_height-docker_margin)
    layout_center_widget().setGeometry(0,measure_height,rect.width()-right_width-docker_margin,rect.height()-measure_height-taskbar_height-title_height-top_docker_height-docker_margin-middle_bottom_height)
...
def getLogPath():
  ...
  path1 = os.path.join(slicerhome,"bin","tmp")
  path2 = os.path.join(slicerhome,"bin","Release","tmp")
  if isPackage():
    return path1
  else:
    return path2
...
def isPackage():
  return not mainWindow().IsPackages()
...
def create_unit_button(pic_path,label,tooltip,buttonsize=64,iconsize=48):
  from FrameworkLib.BottomUnit import BottomUnit
  import qt
  if pic_path:
    unit = BottomUnit(pic_path,label,tooltip,buttonsize,iconsize)
  else:
    unit = qt.QWidget()
    unit.setMaximumWidth(buttonsize)
    unit.setMinimumWidth(buttonsize)
    unit.setMaximumHeight(buttonsize)
    unit.setMinimumHeight(buttonsize)
  return unit

def addWidget2Stretch(parent):
  import qt
...
color_unit_list = None
tips_unit_list = None
def get_unique_color():
  if not color_unit_list:
    return [1,1,1]
  color = color_unit_list.get_unique_color()
  return color
...
def on_btn_toggled1513(toggled,btn,point,number):
    ...
    if toggled:
      ...
      point.AddObserver(slicer.vtkMRMLMarkupsNode.PointPositionDefinedEvent, lambda c,e:on_point_placed1513(c,e,btn,point,number))
    else:
      interactionNode = slicer.app.applicationLogic().GetInteractionNode()
...
def get_button_percent_style(percent):
      style_template = f'''
      QPushButton {{
          /* 渐变背景（同步调整渐变分割点） */
          background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
              stop:0 green,
              stop:{percent/100} green,
              stop:{percent/100} white,
              stop:1 white);
...
def set_button_percent(button,percent):
    if percent == 0:
        global_data_map[button] = {}
        global_data_map[button]['text'] = button.text
        global_data_map[button]['style'] = button.styleSheet
    if percent == 100:
        button.setText(global_data_map[button]['text'])
        button.setStyleSheet(global_data_map[button]['style'])
        return
    style = get_button_percent_style(percent)
...
def reinitForce(background_node=None,foreground_node=None,alpha=0.3,ori=False):
    import slicer
    layoutManager = slicer.app.layoutManager()
...
def loadScene(filename, properties={}):
  ...
  val = None
  try:
    val = loadNodeFromFile(filename, filetype, properties, returnNode=False)
...
def GetVolumeFolderPath(node):
  ...
  if filepath is None:
    default_path = mainWindow().GetPythonBasePath()
    print("default_path:",default_path)
    parent_folder = os.path.dirname(default_path)
    print("parent_folder:",parent_folder)
    output_path = os.path.join(parent_folder,"dce_output")
...
def move_robot_to(val):
  widget = get_robot_arm_module_widget()
  val = widget.move_robot_to(val)
  return val
...
def addPictureFromFile2(filename,image,size_width=80,size_height=80):
  import qt
  img = qt.QImage()
  img.load(filename)
  pixelmap = qt.QPixmap.fromImage(img).scaled(size_width,size_height)
...
def save_to_user_db(key,value):
  from FrameworkLib.datas import fdb
  fdb.save_to_user_db(key,value)

def save_to_global_db(key,value):
  from FrameworkLib.datas import fdb
  fdb.save_to_global_db(key,value)

def get_from_user_db(key,value=None):
  from FrameworkLib.datas import fdb
  return fdb.get_from_user_db(key,value)

def get_from_global_db(key,value=None):
  from FrameworkLib.datas import fdb
  return fdb.get_from_global_db(key,value)

paras = {}
def bind_volumerendering_checkbox(volume_node,checkbox):
  import slicer,vtk
  def on_checkbox_toggled(boolval):
    if checkbox in paras:
      volume = paras[checkbox]
      displayNode = slicer.modules.volumerendering.logic().CreateDefaultVolumeRenderingNodes(volume)
      displayNode.SetVisibility(boolval)
    else:
      print("emtpy checkbox")
  
  @vtk.calldata_type(vtk.VTK_OBJECT)
  def on_display_changed(calldata,_a):
    for key in paras:
      print("paras[key]",paras[key].GetID(),calldata.GetID())
      displayNode = slicer.modules.volumerendering.logic().CreateDefaultVolumeRenderingNodes(paras[key])
      if displayNode == calldata:
        checkbox = key
  ...
  paras[checkbox] = volume_node
  reinit(volume_node)
  checkbox.toggled.disconnect()
  displayNode = slicer.modules.volumerendering.logic().CreateDefaultVolumeRenderingNodes(volume_node)
  if displayNode.GetVisibility():
    checkbox.setChecked(True)
  else:
    checkbox.setChecked(False)
  checkbox.connect('toggled(bool)',on_checkbox_toggled)
  displayNode.AddObserver(vtk.vtkCommand.ModifiedEvent,on_display_changed)
...
def convert_float_to_bytes(val):
    ...
    hex_bytes = [hex(b) for b in bytes_value]
    int_bytes = [int(hex(b),16) for b in bytes_value]
...
def show_file_dialog(file_mode, file_format="",folder_path=None):
  import qt
  res_path = "empty data"
  fileDialog = qt.QFileDialog()
  fileDialog.setFileMode(file_mode)
  if folder_path:
    fileDialog.setDirectory(folder_path)
  if file_format != "":
...
def get_mean_pixel_value_from_segment_to_volume(segmentation_node,volumeNode):
    ...
    while not it.finished:
      ...
      if seg[it.multi_index[0],it.multi_index[1],it.multi_index[2]] == 0:
        ...
        continue
      world = [0, 0, 0, 1]
      ijk = [segImageExtent[0]+it.multi_index[0],segImageExtent[2]+it.multi_index[1],segImageExtent[4]+it.multi_index[2]]
...
def npsave(npy_file_path,dict,savenii=True):
  import numpy as np
  import os
  np.save(npy_file_path,dict)
  if savenii:
    volume = addVolumeFromArray(dict.T)
    # 使用os.path.splitext分离文件名和旧的扩展名
    base_file_path, _ = os.path.splitext(npy_file_path)

    # 添加新的扩展名.nii
    nii_file_path = base_file_path + '.nii'
    print('save nii path:',nii_file_path)
    saveNode(volume,nii_file_path)
    RemoveNode(volume)


def on_camera_direction(direc):
    import slicer
    if direc == "R":
    ...
    if direc == "L":
      ...
      cameraViewUp = [0, 0, 1]
    if direc == "P":
      cameraPosition = [0, -900, 0]
      cameraViewUp = [0, 0, 1]
    if direc == "A":
      cameraPosition = [0, 900, 0]
      cameraViewUp = [0, 0, 1]
    if direc == "S":
      cameraPosition = [0, 0, 900]
      cameraViewUp = [0, 1, 0]
    if direc == "I":
      cameraPosition = [0, 0, -900]
      cameraViewUp = [0, 1, 0]
    ...
    # 获取视图的相机对象
    for camera in getNodesByClass('vtkMRMLCameraNode'):
...
def getNodesByClassByAttribute(classname,attname,attvalue):
  import slicer
  allnodes = []
  nodes = slicer.mrmlScene.GetNodesByClass(classname)
  for node in nodes:
...
def set_color_by_index(node,index):
    color_node = getFirstNodeByClass("vtkMRMLColorNode")
    colors = [0,0,0,0]
    color_node.GetColor(index,colors)
    if node.IsA("vtkMRMLSegmentationNode"):
      segment = GetNthSegment(node,0)
      segment.SetColor(colors[0:3])
    else:
      GetDisplayNode(node).SetColor(colors[0:3])
    return colors[0:3]
...
domino
'''
def set_color_by_name(node,color_name):
  color_node = getFirstNodeByClass("vtkMRMLColorNode")
  ss = color_node.GetNumberOfColors()
  for i in range(ss):
    n1 = color_node.GetColorName(i)
    print(n1)
  index =  color_node.GetColorIndexByName(color_name)
  return set_color_by_index(node,index)
...
def get_roi_by_segmentation(segmentation1):
  ...
  for segmentId in stats["SegmentIDs"]:
    ...
    if not segment:
      print("unfind segment:",segmentation1.GetName(),segmentId,segment)
      continue
...
def set_main_node(volume,force=False):
  scalars = getNodesByClass(vtkMRMLScalarVolumeNode)
  for node in scalars:
...
def CreateDefaultSegmentationNode(name1):
    ...
    if segment_node is None:
      segment_node = CreateNodeByClass("vtkMRMLSegmentationNode")
      segment_node.SetName(name1)
      segment = slicer.vtkSegment()
      segment.SetName(name1)
      segment.SetColor(0.9,0.9,0)
      segment_node.GetSegmentation().AddSegment(segment)
      segment_node.SetScene(slicer.mrmlScene)
      segment_node.CreateDefaultDisplayNodes()
      AddNode(segment_node)
    return segment_node

def CreateMultiDefaultSegmentationNode(name1):
    import slicer
...
def rotate_single_slice_to_planenode(planenode,slice_name):
    import slicer
    normal = list(planenode.GetNormal())
    layoutManager = slicer.app.layoutManager()
    slice_node = layoutManager.sliceWidget(slice_name).mrmlSliceNode()
    matrix = slice_node.GetSliceToRAS()
    pos = [matrix.GetElement(0,3),matrix.GetElement(1,3),matrix.GetElement(2,3)]
    slicer.util.setSlicePoseFromSliceNormalAndPosition(slice_node,normal,pos)
...
def ShowNodeByName(name):
  node = getFirstNodeByName(name)
  if node:
    ShowNode(node)
    
def HideNodeByName(name):
  node = getFirstNodeByName(name)
  if node:
    HideNode(node)
...
def HideNode(node):
  ...
  if node.IsA("vtkMRMLSegmentationNode"):
    displayNode = GetDisplayNode(node)
    if displayNode:
      sid = GetNthSegmentID(node,0)
      displayNode.SetSegmentVisibility(sid,False)
  else:
    displayNode = GetDisplayNode(node)
    if displayNode:
...
def remove_folder_single(folder_name):
  import slicer,vtk
  shNode = slicer.vtkMRMLSubjectHierarchyNode.GetSubjectHierarchyNode(slicer.mrmlScene)
  item = shNode.GetItemByName(folder_name)
  if item == 0:
    print("the folder don't exist")
  else:
    level = shNode.GetItemLevel(item)
    if level == "Folder":
      shNode.RemoveItem(item)
      
def remove_all_folder(folder_name):
  import slicer,vtk
  children = vtk.vtkIdList()
  shNode = slicer.vtkMRMLSubjectHierarchyNode.GetSubjectHierarchyNode(slicer.mrmlScene)
  shNode.GetItemsByName(folder_name,children)
  for i in range(children.GetNumberOfIds()):
    item = children.GetId(i)
    level = shNode.GetItemLevel(item)
    if level == "Folder":
      shNode.RemoveItem(item)
...
def combineModelList2(modellist,output_node):
  ...
  output_node.SetAndObservePolyData(appendFilter.GetOutput())
  return output_node
...
def hideVolumeRendering(volumeNode):
  if not volumeNode:
    return
  import slicer
  volRenLogic = slicer.modules.volumerendering.logic()
...
def GetNthSegment(segmentationNode,i):
  ...
  if not segmentation:
    return None
  return segmentation.GetNthSegment(int(i))
...
def blockSignals(widget,boolval):
  widget.blockSignals(boolval)
...
def easyPlot(array,columnNames,title,tag,chatnode=None,table_node=None,series_Nodes=None,is_unique_color=True,color=[1,1,1]):
  import slicer
  import numpy as np
  #s
...
def easyUpdatePlot(array,tableNode,columnNames,title,tag):
  ...
  nodes = {}
  nodes['table'] = tableNode
  chartNode  = slicer.util.plot(np.vstack(nparrays).T, xColumnIndex = 0,nodes=nodes,columnNames=columnNames, title=title)
  chartNode.SetLegendVisibility(False)
  return tableNode
...
def bind_point_with_button(btn,point_node_name,label=None,archive=False):
    import qt,slicer,vtk
    def on_add_point(val):
      if val:
        point_node = getFirstNodeByName(point_node_name)
        if point_node:
          RemoveNode(point_node)
        point_node = AddNewNodeByClass(vtkMRMLMarkupsFiducialNode)
        point_node.SetName(point_node_name)
        
        display_node = GetDisplayNode(point_node)
    ...
    def on_point_defined(pointnode,event):
      ...
      btn.setChecked(False)
      if label:
        display_node = GetDisplayNode(pointnode)
...
def bind_point_with_img_button(btn, point_node_name, label, normal_img, define_img, archive=False):
    import qt,slicer,vtk
    def on_add_point(val):
      if val:
        point_node = getFirstNodeByName(point_node_name)
    ...
    def on_point_defined(pointnode,event):
      if pointnode is None:
        return
      btn.styleSheet = btn.styleSheet + f"background-image: url({define_img});"
    ...
    def on_point_removed(point_node,event):
      #print("on_point_removed",point_node.GetID(),event)
      btn.setChecked(False)
      btn.styleSheet = btn.styleSheet + f"background-image: url({normal_img});"
      btn.setStyleSheet(btn.styleSheet)
...
def showRealWarningText(text, width=530, height=300):
  return getModuleWidget("JMessageBox").show_infomation(tr('警示信息'), text, 1, width, height)
  
def showWarningText(text, width=530, height=300):
  return getModuleWidget("JMessageBox").show_infomation(tr('提示信息'), text, 0, width, height)
  
def showWaitText(text, title='', width=530, height=300):
  if title == '':
    title = tr('等待信息')
  return getModuleWidget("JMessageBox").show_infomation(title, text, 3, width, height)

def showSuccessText(text, title='', width=530, height=300):
  if title == '':
    title = tr('成功信息')
  return getModuleWidget("JMessageBox").show_infomation(title, text, 4, width, height)

def showInformationText(text, title='', width=530, height=300):
  if title == '':
    title = tr('提示信息')
  return getModuleWidget("JMessageBox").show_infomation(title, text, 0, width, height)

def showErrorText(text, title='', width=530, height=300):
  if title == '':
    title = tr('错误信息')
  return getModuleWidget("JMessageBox").show_infomation(title, text, 2, width, height)
...
def create_log_file(directory_path="D:/"):
    ...
    module_name = directory_path.split("\\")[-2]
    # 指定日志文件的路径（可根据需要修改）
    log_directory = os.path.join(mainWindow().GetPythonBasePath(),'UnitTestLog','Latest',module_name) 
    #log_directory = os.path.join(directory_path,"log",date_time)
    # 确保日志目录存在，如果不存在则创建
    ...
    print("create log path:",unit_test["log_file_path"])
    # 创建并打开日志文件
...
def messageBox(text, parent=None, **kwargs):
  ...
  for key, value in kwargs.items():
    if key == 'windowTitle':
      windowTitle = value
  result = getModuleWidget("JMessageBox").show_question(windowTitle,text)
  return result


def messageBox2(text, width=530, height=300, parent=None,title="消息", **kwargs):
  """Displays a messagebox.
...
def log_screen_shot(module_directory,name):
  import ScreenCapture,os
  cap = ScreenCapture.ScreenCaptureLogic()
  filefolder = os.path.dirname(unit_test["log_file_path"])
  filename = os.path.join(module_directory,filefolder,name)
...
def cloneAttributes(fromnode,tonode):
  for attr in fromnode.GetAttributeNames():
    tonode.SetAttribute(attr, fromnode.GetAttribute(attr))

def convert_segment_to_labelmap(segmentation_node):
  import slicer
  labelmapVolumeNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLLabelMapVolumeNode")
  slicer.modules.segmentations.logic().ExportAllSegmentsToLabelmapNode(segmentation_node, labelmapVolumeNode, slicer.vtkSegmentation.EXTENT_REFERENCE_GEOMETRY)
...
def convert_segment_to_model(segmentation_node):
  ...
  shNode = slicer.vtkMRMLSubjectHierarchyNode.GetSubjectHierarchyNode(slicer.mrmlScene)
  exportFolderItemId = shNode.CreateFolderItem(shNode.GetSceneItemID(), segmentation_node.GetName())
...
def on_landing_to_model(fiducialNode,modelNode):  
    import vtk
    # 假设我们关注第一个标记点
    point = [0,0,0]
    ...
    # 创建 vtkCleanPolyData 实例并设置其输入为模型的 vtkPolyData
    cleanFilter = vtk.vtkCleanPolyData()
    cleanFilter.SetInputData(polyData)

    # (可选) 配置 vtkCleanPolyData 的参数，如下所示：
    # cleanFilter.SetTolerance(0.01)  # 设置合并顶点的容差
    # cleanFilter.PointMergingOn()  # 开启点合并
    ...
    # 使用清理后的 vtkPolyData 更新 vtkMRMLModelNode
    modelNode.SetAndObservePolyData(cleanedPolyData)
...
def on_keep_model_largest_island(modelNode):
    import vtk
    modelNode = getFirstNodeByClassByName(vtkMRMLModelNode,"YellowCircleModel")
    # 假设 modelNode 是你的 vtkMRMLModelNode
    polyData = modelNode.GetPolyData()
    # 创建一个连接性过滤器并设置它的输入
    connectivityFilter = vtk.vtkConnectivityFilter()
    ...
    for i in range(connectivityFilter.GetNumberOfExtractedRegions()):
        ...
        regionSize = connectivityFilter.GetOutput().GetNumberOfCells()  # 或者 GetNumberOfPoints()
        regionSizes[i] = regionSize
        connectivityFilter.DeleteSpecifiedRegion(i)

    # 找到最大连通域
    largestRegion = max(regionSizes, key=regionSizes.get)

    # 设置提取模式为提取指定区域，并设置最大连通域为目标区域
    connectivityFilter.SetExtractionModeToSpecifiedRegions()
    connectivityFilter.InitializeSpecifiedRegionList()
    connectivityFilter.AddSpecifiedRegion(largestRegion)
    connectivityFilter.Update()

    # 获取只包含最大连通域的 polyData
    largestRegionPolyData = connectivityFilter.GetOutput()
    # 更新 vtkMRMLModelNode
    modelNode.SetAndObservePolyData(largestRegionPolyData)
...
def convert_model_to_segment(model_node,master_node):
  ...
  # return seg
  return convert_model_to_segment_accuracy(model_node)
...
def convert_model_to_segment_accuracy(model_node,spacing=1):
    import slicer,vtk
    segmentationNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLSegmentationNode")
    segmentationNode.CreateDefaultDisplayNodes() 

    geometryImageData = slicer.vtkOrientedImageData()
    geometryImageData.SetSpacing(spacing,spacing,spacing)
...
def save_cache_to_PAAA(key,value):
  import qt,os
  ...
  path = os.path.join(target_path,"settings.ini").replace("\\","/")
  config_file = qt.QSettings(path,qt.QSettings.IniFormat)
  config_file.setIniCodec("UTF-8")
  config_file.setValue(key, value)

def get_cache_from_PAAA(key,default=None):
  import qt,os
  storage_dir = qt.QStandardPaths.writableLocation(qt.QStandardPaths.GenericDataLocation)
  target_path = f'{storage_dir}/paaa'
  if not os.path.exists(target_path):
...
def GetScalarRange(volume):
  import numpy as np
  arr = arrayFromVolume(volume)
  hi = np.max(arr)
  lo = np.min(arr)
  return lo,hi
...
def get_folder_path():
  res_path,result = show_file_dialog(2)
  if result == 0:
    return ""
  else:
    return res_path
...
def update_setting_from_config(sett):
  ...
  slicer.util.QSS_QWidget_background_color = settingsValue("General/QSS_QWidget_background_color","#000000",settings=sett)
...
def show_screen_label(str1):
  import qt,slicer
  mainWindow().ShowGlobalPopup(str1,1000)
...
def test_file_create(folder_path,filename):
  # 保存工作簿
    ...
    ws.append([t0, '用例编号', '测试类型', '测试方式', '测试用例描述', '测试输入数据', '测试执行步骤', '测试预期结果', '测试记录', '判定', '测试人', '测试日期', '备注'])
...
def _generate_comment_doc(comment_class_name,class_obj,comment_path):
  ...
  for function_name, function_obj in members:
      if not function_name.startswith('__') and not function_name.startswith('_') and function_obj.__qualname__.startswith(class_obj.__name__):
...
def get_patient_id():
  ...
  return patient_id
def _get_patient_id():
  nodes = getNodesByClass(vtkMRMLScalarVolumeNode) + getNodesByClass(vtkMRMLDiffusionWeightedVolumeNode)
  if len(nodes)==0:
    return
  sorted_nodes = sorted(nodes, key=lambda cab: cab.GetName())
  for i in range(len(sorted_nodes)):
    node = sorted_nodes[i]
    boolval,patient_id = get_patient_id_by_node(node,True)
    if boolval:
      SetGlobalSaveValue("patient_id",patient_id)
      return patient_id
  if GetGlobalSaveValue("patient_id") is not None:
    return GetGlobalSaveValue("patient_id")
  for i in range(len(sorted_nodes)):
    node = sorted_nodes[i]
    boolval,patient_id = get_patient_id_by_node(node,False)
    if boolval:
      SetGlobalSaveValue("patient_id",patient_id)
      return patient_id
  patient_id = generate_random_string()
  SetGlobalSaveValue("patient_id",patient_id)
  return patient_id
  
def map_to_fixed_length(input_string):
    import hashlib
    # 创建一个 SHA-256 哈希对象
    sha256 = hashlib.sha256()

    # 将输入字符串编码为字节对象并更新哈希对象
    sha256.update(input_string.encode('utf-8'))

    # 获取哈希结果并取前10个字符
    hashed_result = sha256.hexdigest()[:10]

    return hashed_result
  
def get_patient_id_by_node(node,val):
  import slicer,os
  if val:
    if node.GetNodeTagName() == "DiffusionWeightedVolume":
      str1 = ""
      for attr in node.GetAttributeNames():
        str1 = str1+node.GetAttribute(attr)
      if str1!="":
        patient_id = map_to_fixed_length(str1)
        return True,patient_id
    if node.GetAttribute("DICOM.instanceUIDs"):
      instUids = node.GetAttribute("DICOM.instanceUIDs").split()
      filename = slicer.dicomDatabase.fileForInstance(instUids[0])
      patient_id = slicer.dicomDatabase.fileValue(filename,"0010,0020")
      if patient_id is not None:
        return True,patient_id
      patient_id = slicer.dicomDatabase.fileValue(filename,"0008,0031")
      if patient_id is not None:
        return True,patient_id
      patient_id = slicer.dicomDatabase.fileValue(filename,"0010,0010")
      if patient_id is not None:
        return True,patient_id
    else:
      return False,""
  else:
    path = node.GetAttribute("filepath")
    if path and os.path.exists(path):
      instUids = calculate_sha256(path)
      return True,instUids
    else:
      return False,""
...
def calculate_sha256(file_path):
    ...
    return sha256_hash.hexdigest()[0:10]
...
def get_comment_prompt(keys,source_code):
  ...
  question_list.append({"role":"user","content":"Please use chinese to provide a detailed comment above the function, explaining its purpose(using the format '@detail  :purpose description'), the parameters(using the format '@para : description'), and the return value(using the format '@return : description'). '%s'"%(source_code)})
...
def do_load_jmodule(module_name_pack,scriptedDirPath,moduleFactoryManager,is_c=False,is_d=False):
  ...
  if is_c:
    if is_d:
      module_path = (os.path.join(scriptedDirPath,'DExtension', 'CExtension',module_name,'qt-scripted-modules')).replace('\\','/')
      moduleFactoryManager.pyAddSearchPath(module_path)
      module_path = (os.path.join(scriptedDirPath,'DExtension', 'CExtension',module_name,'qt-loadable-modules')).replace('\\','/')
      moduleFactoryManager.pyAddSearchPath(module_path)
      module_path = (os.path.join(scriptedDirPath,'DExtension', 'CExtension',module_name,'cli-modules')).replace('\\','/')
      moduleFactoryManager.pyAddSearchPath(module_path)
      module_conifg_path = (os.path.join(scriptedDirPath,'DExtension', 'CExtension',module_name,"Resources","config.ini")).replace('\\','/')
    else:
      ...
      moduleFactoryManager.pyAddSearchPath(module_path)
      module_conifg_path = (os.path.join(scriptedDirPath, 'CExtension',module_name,"Resources","config.ini")).replace('\\','/')
  else:
    if is_d:
...
def add_search_paths(module_names,c_module_names,r_module_names,filepath):
    print("VVVVVVVVVVVVVVVVVVVVVVV:",module_names,c_module_names,r_module_names,filepath)
    import slicer,os,qt
    moduleManager = slicer.app.moduleManager()
    moduleFactoryManager = moduleManager.factoryManager()
    scriptedDirPath = os.path.abspath(os.path.dirname(filepath))
...
def shut_down():
  result = getModuleWidget("JMessageBox").show_question(tr("提示"), '是否要关闭计算机？ ')
  print(result)
  if result == 0:
    return
...
def show_vip():
  dialog = getModuleWidget("JMessageBox").on_popup_ui_dialog("成为VIP",getModuleWidget("PayForVip").get_widget(), 700, 490)
  getModuleWidget("PayForVip").set_dialog(dialog)
  dialog.exec_()

def get_common_save_path(save_type):
  import os
  import qt
  storage_dir = qt.QStandardPaths.writableLocation(qt.QStandardPaths.GenericDataLocation)
  target_path = f'{storage_dir}/paaa'
  if not os.path.exists(target_path):
    os.mkdir(target_path)
  target_path = f'{storage_dir}/paaa/{save_type}'
  if not os.path.exists(target_path):
    os.mkdir(target_path)
  return target_path
...
def show_update_reminder(version, info):
  print("show_update_reminder", version, info)
  from FrameworkLib.ReleaseDialog import ReleaseDialog
  dialog = ReleaseDialog(mainWindow())
  dialog.set_info(version, info)    
  dialog.show()
  result = dialog.exec_()
  return result
...
def tr(info):
  ...
  res = QCoreApplication.translate("ZZExContent", info)
  return res
